...
#if defined(NDEBUG)
# error "Chroncoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x080e31b6457eaeab9e71e5e09b76d15c457e82e1ce2940a0fee702a4d298a023");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Chroncoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Chroncoin Signed Message:\n";
...
{
    // Chroncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Chroncoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Chroncoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Chroncoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Chroncoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Chroncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xcfaa50ad5c1009d30a55d31447017d9c92db6b6d7b9f99511439b1b4db1cf31d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "CNN JAN 29 2018 FBI Director Steps Down";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1517300190;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084868153;

        if (fTestNet)
        {
            block.nTime    = 1517300178;
            block.nNonce   = 387665826;
        }
//THIS IS ALL SPECIAL CODE FROM THE YOUTUBE
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xeacabd31ef22a9a4d23528aba82a251fc73c41a474a1b4efbe0f49a4a314c8e4"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc4, 0xb1, 0xda }; // Chroncoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ChroncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ChroncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ChroncoinMiner(CWallet *pwallet)
{
    printf("ChroncoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("chroncoin-miner");
    ...
    try { loop {
        ...
        printf("Running ChroncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ChroncoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ChroncoinMiner, pwallet));
}
